...
|---------|----------|------|-------|
| **highlight.js** | Most websites | ~2KB | Simple API, many themes |
...
Shiki is ideal for static site generators and build-time highlighting:
...
All three libraries support popular dark themes:

| Theme | highlight.js | Prism.js | Shiki |
|-------|-------------|----------|-------|
| GitHub Dark | `github-dark` | `prism-theme-github-dark` | `github-dark` |
| Dracula | `dracula` | `prism-dracula` | `dracula` |
| One Dark | `atom-one-dark` | `prism-one-dark` | `one-dark-pro` |
| Nord | `nord` | `prism-nord` | `nord` |

## Framework Integration

### Astro / Starlight

For Astro sites using Starlight, you can configure the Jake grammar in `ec.config.mjs`. See the [Jake documentation site source](https://github.com/helgesverre/jake) for an example.

### Next.js / React

Use Shiki with `@shikijs/rehype` for MDX content:
...
Use highlight.js with `@highlightjs/vue-plugin`:
...
<template>
  <highlightjs language="jake" :code="code" />
</template>
```
