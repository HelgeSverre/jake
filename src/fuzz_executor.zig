const std = @import("std");
const jake = @import("jake");

// Fuzzing harness for executor (dry-run mode).
//
// Designed to be used with external fuzzers (AFL++, honggfuzz, etc.) by compiling
// with Zig's `-ffuzz` instrumentation.
//
// The harness parses the input and attempts to execute in dry-run mode (no shell commands).
// This tests dependency resolution, variable expansion, and conditional evaluation.
pub fn main() !void {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();

    const args = try std.process.argsAlloc(allocator);
    defer std.process.argsFree(allocator, args);

    const max_len = 1024 * 1024;

    const input = blk: {
        if (args.len >= 2) {
            break :blk try std.fs.cwd().readFileAlloc(allocator, args[1], max_len);
        }
        break :blk try std.fs.File.stdin().readToEndAlloc(allocator, max_len);
    };
    defer allocator.free(input);

    // Parse the input
    var lex = jake.Lexer.init(input);
    var p = jake.Parser.init(allocator, &lex);
    var jakefile = p.parseJakefile() catch return;
    defer jakefile.deinit(allocator);

    // Execute in dry-run mode
    var exec = jake.Executor.init(allocator, &jakefile);
    defer exec.deinit();
    exec.dry_run = true;

    // Try to execute the first recipe or default
    if (jakefile.recipes.len > 0) {
        // Try executing the first recipe
        exec.execute(jakefile.recipes[0].name) catch {};
    }
}
