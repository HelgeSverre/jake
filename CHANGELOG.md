...
- **`silent` is now a reserved word.** Like `quiet`, `hidden`, and `timeout` before it, `silent` is a lexer keyword, so a bare top-level `silent:` recipe or a `silent = ...` variable no longer parses. `task silent:` still works, as does `silent` as a parameter name, a dependency, or a hook target.
