...
- **`--silent` and `@silent` suppress jake's own status chrome (jake#37).** jake prints a `→ name` start line, a `✓ name <time>` completion line, and a `Successfully ran N tasks / Total time` footer to stderr around every recipe. The new `--silent` flag (and `JAKE_SILENT` env var) hides that chrome for the whole run, while the recipe-level `@silent` directive hides it for one recipe; the recipe's stdout/stderr, error diagnostics, exit codes, listings, watch chrome, and the web UI are unchanged. `@silent` also implies `@quiet`, so it hides `-v` command echoing too. `--silent` is stronger than the existing `@quiet` (which only suppresses command echoing) and is deliberately not named `--quiet` to avoid overloading that word.

### Changed

- **A recipe directive with no recipe after it is now a parse error (breaking).** `@quiet`, `@silent`, `@hidden`, `@desc`, `@group`, `@alias`, `@platform`, `@timeout` and `@needs` apply to the recipe that follows them, but nothing stopped an unrelated top-level element from coming between: a variable assignment, an `@import`, an `@export`/`@dotenv`, or a global hook would pass the pending directive straight through to whatever recipe came next, and a directive at the end of the file was discarded without a word. `jake --fmt` made that worse than cosmetic — it reorders by category, so the intervening element was moved out of the way and the wrong association was written back into the source permanently, while a trailing directive was deleted outright. All nine now report `expected recipe after '@<name>'`, the wording `@default` has always used for exactly this. Stacking directives on one recipe is unaffected: only a *non-directive* element, or end of file, breaks the chain.

### Fixed

- **Keywords work as top-level recipe and variable names.** `silent:`, `quiet:`, `hidden:`, `timeout = "30"` and the like failed with `unexpected token at top level`, because the top-level dispatch routed only plain identifiers into the variable/recipe parser while every other position in the grammar already accepted keywords as names (`task silent:`, `silent` as a parameter, a dependency, or a hook target all worked). Adding `@silent` made this visible; it was true of every keyword. The dispatch now consults the same `isNameToken` helper the rest of the parser uses, so adding a directive no longer reserves a word.

- **`--silent` now covers watch mode, and `-n --silent` stays readable.** Watch mode's own output — the `watching` banner, the `changed` notice, the per-cycle footer — ignored the flag, so `jake -w --silent` still printed chrome around every rebuild (the executor's own chrome was already suppressed, since watch mode shares the same context). Rebuild diagnostics are deliberately kept: a silent watch still says why a build broke. Separately, `-n --silent` dropped the `○ recipe` headers and left a bare list of commands with nothing to attribute them to; dry-run prints nothing but commands, so the header is part of the payload rather than chrome and is now kept.

- **`@timeout` could SIGKILL a process group jake does not own.** The watchdog thread was handed `&child` — a pointer to a `std.process.Child` living on `runCommandWithTimeout`'s stack frame, which the command loop re-enters for every command in the recipe. If the watchdog observed the deadline and loaded that pointer, then lost its slice while the main thread reaped the command and started the next one, it dereferenced a reinitialized frame: `Child.init` leaves `.id = undefined`, so it read whatever was at that offset and passed it to `kill(-pid, SIGKILL)`. The likely value was the previous command's reaped pid — SIGKILL to whatever had since recycled it — and a garbage `0` or `1` meant jake's own process group or every process the user could signal. It now publishes the pid instead of a pointer, the way `CancellationMonitor` already did, so there is nothing to dereference; every kill path also routes through one helper that refuses pids below 2. Reachable from any `@timeout` recipe with more than one command; needed the deadline to land near a command boundary.

- **`jake --fmt` no longer deletes `@hidden`.** `renderRecipe` emitted `@group`, `@desc`, `@alias`, `@quiet`, `@timeout`, `@platform` and `@default` but never `@hidden`, so formatting a Jakefile silently un-hid every recipe that used the directive — it reappeared in `jake --list` and in shell completions. Same class as the `@rooted` loss fixed in 0.9.8.
...
- **Formatter: a file-level `@rooted` directive is no longer dropped (jake#30).** `jake --fmt` deleted `@rooted` from any file that declared it, which silently unroots the module: an imported Jakefile then resolves `@cd`, `file` targets, and relative paths against the importer's directory instead of its own. `@rooted` is a module-level flag, so the parser records a bool on `Jakefile` rather than a `Directive` node, and `renderDirective` — which knows only `@dotenv`, `@require`, and `@export` — had nothing to emit. The parser now also records the directive's source line, and the formatter re-emits `@rooted` at the top of the file with its source-owned comment block while keeping detached file headers above it (jake#29).

- **Formatter: file-header and directive comments are no longer moved onto the first recipe (jake#29).** `jake --fmt` drained every standalone comment above the first recipe into one block glued to that recipe, so a file header, a comment documenting `@dotenv`/`@import`, and section separators were concatenated and the directive they documented was left bare. The 0.9.0 fix only stopped a _later_ recipe from taking an _earlier_ recipe's comment; the first recipe still absorbed everything above it, because top-level elements carried no source line to bound the drain against. Imports, directives, variables, and hooks now record their 1-based source line, and each formatter element consumes only the comments between it and the preceding source element. Comments therefore remain attached even when imports, directives, and `@rooted` are canonically reordered; file headers stay at the top, and comments between variables split them into independently aligned groups instead of moving to the end. A comment followed by a blank line keeps that blank line, and a comment block below the last import stays with the import list. Blank lines no longer double up, and the output ends with exactly one newline, so a second `--fmt` pass is a no-op. `jake --fmt` on jake's own `Jakefile` is now a no-op.
